...
/**
 * 펫 캐릭터 렌더러 — 우클릭 메뉴에서 선택한 캐릭터(페이지/고스트/사용자 이미지)를 표시한다.
 * 기본: 노션 AI 스타일 문서 페이지 + AI 스파클 (오리지널 디자인, 잉크 스타일)
 */
export default function Pet({ onClick }: PetProps) {
  const state = usePetStore((s) => s.state);
  const kind = useCharacterStore((s) => s.kind);
  const customImage = useCharacterStore((s) => s.customImage);
  ...
  return (
    ...
    >
      {kind === "custom" && customImage ? (
        ...
        >
          {kind === "ghost" ? <GhostBody /> : <PageBody state={state} />}
          <Face state={state} ghost={kind === "ghost"} />
        </svg>
      )}
    </div>
  );
...
/** 몸통: 모서리 접힌 문서 페이지 + AI 스파클 */
function PageBody({ state }: { state: string }) {
  return (
    <>
      <path
        d="M36 14
...
/** 표정 + 상태 소품 (볼터치/눈/입/zzz/연필) */
function Face({ state, ghost }: { state: string; ghost: boolean }) {
  const cheekY = ghost ? 66 : 70;
  const cheekL = ghost ? 34 : 40;
  const cheekR = ghost ? 86 : 80;
  return (
    <>
      <ellipse cx={cheekL} cy={cheekY} rx="7" ry="4.5" fill="#f5c8c2" />
      <ellipse cx={cheekR} cy={cheekY} rx="7" ry="4.5" fill="#f5c8c2" />
      {state === "sleeping" ? (
        <g>
          <path
